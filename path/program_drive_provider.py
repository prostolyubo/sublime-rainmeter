...
def get_cached_program_drive():
	...
	if not rainmeterpath:
		logger.log(__file__, "get_cached_program_drive", "Rainmeter program path not found.")
		return

	probe_drive = os.path.splitdrive(rainmeterpath)
	if not probe_drive:
		logger.log(__file__, "get_cached_program_drive", "Drive could not be extracted from '"+rainmeterpath+"'.")
		return
